...
export class MultichainSDK extends EventEmitter<SDKEvents> implements MultichainSDKBase {
	...
	private constructor(options: MultichainSDKConstructor) {
		...
		const sdkInfo = `Sdk/Javascript SdkVersion/${packageJson.version} Platform/${platformType} dApp/${this.options.dapp.url ?? this.options.dapp.name} dAppTitle/${
			this.options.dapp.name
		}`;
		this.provider = getMultichainClient({ transport: this.transport });
		this.rpcClient = new RPCClient(this.provider, this.options.api, sdkInfo);
	}

	private get transport() {
		const platformType = getPlatformType();
		if (platformType === PlatformType.DesktopWeb || platformType === PlatformType.MetaMaskMobileWebview || platformType === PlatformType.MobileWeb) {
	...
	async connect(scopes: Scope[], caipAccountIds: CaipAccountId[]): Promise<SessionData> {
		if (!this.transport.isConnected) {
			await this.transport.connect();
		}

		const optionalScopes = scopes.reduce<OptionalScopes>(
			(prev, scope) => ({
				...prev,
				[scope]: {
					methods: [],
		...
		if (session) {
			...
			const scopesMatch = existingScopes.every((scope) => scopes.includes(scope as Scope)) && scopes.every((scope) => existingScopes.includes(scope));
