import { ConnectionStatus, EventType, MetaMaskSDK, SDKProvider } from '@metamask/sdk';
...
const createStyles = (connectionStatus: ConnectionStatus) => {
   return StyleSheet.create({
      container: {
         borderWidth: 2,
         borderColor: connectionStatus === ConnectionStatus.LINKED ? colors.success.default : colors.warning.default,
         padding: 10,
...
export const DAPPView = ({ sdk }: DAPPViewProps) => {
   ...
   //    }
   //    const bal =
   //       (await provider?.getBalance(sdk.getProvider()?.selectedAddress as string)) ?? ethers.BigNumber.from(0);
   ...
   useEffect(() => {
      ...
      try {
         console.debug(`useffect ethereum.selectedAddress=${sdk.getProvider()?.selectedAddress}`);
         if (sdk.getProvider()?.selectedAddress) {
            setConnected(true);
         ...
         sdk.on(EventType.CONNECTION_STATUS, (_connectionStatus: ConnectionStatus) => {
   ...
   const sign = async () => {
      const msgParams = JSON.stringify({
         domain: {
            ...
            chainId: sdk.getProvider()?.chainId ? parseInt(sdk.getProvider()?.chainId || '', 16) : 1,
