...
export const handleChannelRejected = async (
  ...
  try {
    // Get existing config using pubClient wrapper
    ...
    if (channelConfig) {
      ...
      if (channelConfig.ready) {
        ...
        callback?.(null, { success: false, reason: 'channel_already_ready' });
        return;
      }
    ...
    // Update redis channel config to inform dApp of rejection using pubClient wrapper
    ...
    logger.error(
      `[handleChannelRejected] Error for channelId=${channelId}: ${error}`,
      {
        channelId,
        socketId,
        clientIp,
