...
import { CaipAccountId, CaipChainId, CaipAccountAddress, parseCaipAccountId, Json } from "@metamask/utils";
    ...
    >
      ...
      >
        ...
        <div className="p-4 bg-white border-t border-gray-200">
          <div className="mb-2">
            <label className="block text-sm font-medium text-gray-600 mb-1">JSON Request:</label>
          </div>
          <textarea
            data-testid={`${escapeHtmlId(scope)}-collapsible-content-textarea`}
            value={invokeMethodRequests[scope] ?? ""}
            onChange={(evt) =>
              setInvokeMethodRequests((prev) => ({ ...prev, [scope]: evt.target.value }))
            }
            rows={12}
          ...
          <div className="mt-2 text-xs text-gray-500">
            Edit the JSON above to customize the request parameters before invoking the method.
          </div>
        </div>
      </details>
          ...
          return results.map(({ result, request }, index) => {
            ...
            return truncated ? (
              ...
              >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`font-mono text-sm font-medium ${isError ? 'text-red-700' : 'text-purple-700'}`}>
                          {method}
                        </span>
                ...
                <div className="p-4 bg-white border-t border-gray-200">
                  <div className="mb-2">
                    <label className="block text-sm font-medium text-gray-600 mb-1">Full Response:</label>
                  </div>
                ...
                <div className="p-4">
                  <div className="relative">
                    <pre className="bg-gray-50 p-4 rounded-md text-sm font-mono overflow-x-auto border border-gray-200 text-left">
                      <code
                        id={`invoke-method-${escapeHtmlId(scope,)}-${method}-result-${index}`}
                        className={isError ? 'text-red-600' : 'text-gray-800'}
                      >
                        {text}
                      </code>
